// 기존 앵커 1번(왼쪽) DWM3000 코드. 왼쪽 보드는 CS 핀이 2번이다.
#include <SPI.h>
#include "dw3000.h"
const uint8_t PIN_RST=5, PIN_IRQ=4, PIN_SS=2;
...
static uint8_t tx_poll_msg[]={0x41,0x88,0,0xCA,0xDE,0xAA,0x00,0x01,0x00,0xE0,0,0};
...
void setup(){
  ...
  Serial.println("UWB left anchor ready");
}
void loop(){
  ...
  if(dwt_starttx(DWT_START_TX_IMMEDIATE|DWT_RESPONSE_EXPECTED)!=DWT_SUCCESS){
    Serial.println("ANCHOR1 TX START FAILED");
  ...
  if(!timeout_occurred&&(status_reg&SYS_STATUS_RXFCG_BIT_MASK)){
    ...
    if(frame_len>=18&&frame_len<=sizeof(rx_buffer)){
      ...
      if(rx_buffer[9]==0xE1&&rx_buffer[5]==0x01&&rx_buffer[7]==0xAA){
        ...
        int32_t rtd_init=resp_rx_ts-poll_tx_ts,rtd_resp=resp_tx_ts-poll_rx_ts;
        float distance=(((rtd_init-rtd_resp*(1-clockOffsetRatio))/2.0)*DWT_TIME_UNITS)*SPEED_OF_LIGHT;
        Serial.print("ANCHOR1,"); Serial.println(distance);
      }
    }
  }else if(status_reg&SYS_STATUS_ALL_RX_ERR){
    Serial.print("ANCHOR1 RX ERROR 0x"); Serial.println(status_reg, HEX);
    dwt_write32bitreg(SYS_STATUS_ID,SYS_STATUS_ALL_RX_ERR);
  }
  if(timeout_occurred){
    Serial.println("ANCHOR1 NO RESPONSE");
  }
